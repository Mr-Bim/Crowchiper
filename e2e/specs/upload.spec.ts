...
/** Progress stages that should appear during upload */
...
test.describe("Image upload", () => {
  ...
  test("shows progress feedback during upload", async () => {
    ...
    // Set up mutation observer to capture progress text changes
    await page.evaluate(() => {
      (window as unknown as { __progressTexts: string[] }).__progressTexts = [];
      const observer = new MutationObserver(() => {
        const uploadingElements = document.querySelectorAll(
          ".cm-attachment-uploading",
        );
        uploadingElements.forEach((el) => {
          const text = el.textContent?.trim();
          if (
            text &&
            !(
              window as unknown as { __progressTexts: string[] }
            ).__progressTexts.includes(text)
          ) {
            (
              window as unknown as { __progressTexts: string[] }
            ).__progressTexts.push(text);
          }
        });
      });
    ...
    // Get captured progress texts
    const progressTexts = await page.evaluate(
      () =>
        (window as unknown as { __progressTexts: string[] }).__progressTexts,
    );

    // Verify we saw at least one progress indicator
    expect(progressTexts.length).toBeGreaterThan(0);

    // Check that we saw relevant progress stages (at least compressing or uploading)
    const sawProgressStage = PROGRESS_STAGES.some((stage) =>
      progressTexts.some((text) => text.includes(stage)),
    );
    expect(sawProgressStage).toBe(true);
  });
