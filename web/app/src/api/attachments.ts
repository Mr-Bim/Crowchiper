 ...
 */
export async function getAttachment(uuid: string): Promise<BinaryAttachmentResponse> {
...
export async function getAttachmentThumbnail(
	...
	const response = await fetch(`${API_PATH}/attachments/${uuid}/thumbnail/${size}`, {
		credentials: "include",
	});
...
export async function getAttachmentThumbnails(
	...
	if (!response.ok) {
		const errorMsg = await getErrorMessage(response, "Failed to get thumbnails");
		throw new Error(errorMsg);
	}
