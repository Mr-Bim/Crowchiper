...
function getElements() {
	return {
		...
		testUsername: document.getElementById("test-username") as HTMLInputElement,
...
async function testPrfSupport(username?: string): Promise<boolean> {
	...
	// Check if PRF extension returned a result
	const prfResult = (
		credential.clientExtensionResults as {
			prf?: { results?: { first?: ArrayBuffer } };
		}
	)?.prf;
	return prfResult?.results?.first != null;
}
...
async function handleTestPrf(
	...
	try {
		// Get username from hidden test field (for automated testing)
		const username = elements.testUsername.value.trim() || undefined;
