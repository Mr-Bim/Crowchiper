...
- **Tests**: Rust tests in `tests/`. E2E in `e2e/` (see `e2e/CLAUDE.md`). PRF injection for WebAuthn testing. Plugin tests in `tests/plugin_tests.rs` (45 tests) and `tests/plugin_permission_isolation_tests.rs` (7 tests verifying permissions don't leak between plugins).
...
- **Plugin hooks**: Plugins export `on-hook(event) -> result<_, string>` to react to server events. Hooks are WIT enums grouped by target: `variant hook { server(server-hook) }` with `server-hook { ip-change }`. Each plugin declares a `target` (server only for now) in `plugin-config` and registers specific hooks — validated at load time to match target. `PluginManager` dispatches hooks: different plugins run in parallel (`std::thread::scope`), same plugin serializes (Mutex per plugin, WASM is single-threaded). Called via `tokio::task::spawn_blocking` from async code. First hook: `ip-change` fired in `src/auth/extractors.rs` when user IP changes during token refresh (values: `old_ip`, `new_ip`, `user_uuid`).
